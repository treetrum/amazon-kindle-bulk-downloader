 ...
 */
const getKindleDevice = async (auth) => {
 ...
 */
const getAllContentItems = async (auth) => {
    const data = (await fetch(`${OPTIONS.baseUrl}/hz/mycd/digital-console/ajax`, {
        headers: getHeaders(auth),
        body: new URLSearchParams({
            csrfToken: auth.csrfToken,
 ...
 */
const getDownloadUrl = async (auth, device, asin) => {
 ...
 */
const downloadSingleBook = async (auth, device, book, progressBar) => {
 ...
 */
const downloadBooks = async (auth, device, books) => {
    ...
    for (let index = 0; index < bookChunks.length; index++) {
        ...
        await Promise.all(chunk.map((b) => downloadSingleBook(auth, device, b, progressBar)));
        progressBar.stop();
    }
};
/**

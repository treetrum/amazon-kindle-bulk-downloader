...
const downloadSingleBook = async (auth, device, book, progressBar, options) => {
    ...
    if (response.ok) {
        ...
        promises_1.default.mkdir(downloadsDir, { recursive: true });
        await promises_1.default.writeFile(path_1.default.join(downloadsDir, filename), Buffer.from(data));
    }
    else {
...
const downloadBooks = async (auth, device, books, options) => {
    ...
    await limitConcurrency(books
        .slice(options.startFromOffset, options.totalDownloads)
        .map((b) => () => downloadSingleBook(auth, device, b, progressBar, options)), options.maxConcurrency);
    progressBar.stop();
};
/**
